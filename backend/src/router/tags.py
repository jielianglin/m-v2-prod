...
def get_network(session: Session = Depends(get_session)):
    ...
    result = session.execute(
        'SELECT a.tag_id "source", b.tag_id "target", COUNT(*) "value" FROM images_tags a	JOIN images_tags b ON b.image_id = a.image_id	AND b.tag_id > a.tag_id	GROUP BY a.tag_id,		b.tag_id')
    links = []
    for row in result:
        links.append(row)
    result = session.execute(
        'SELECT ALL images_tags.tag_id id,t.tag "name",COUNT(*) "value" FROM images_tags LEFT JOIN tags t ON images_tags.tag_id=t.id GROUP BY images_tags.tag_id')
    nodes = []
    for row in result:
        nodes.append(row)
